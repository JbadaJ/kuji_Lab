...
**Improvement roadmap (from 2026-08-04 audit)**: P0 room-server security is **DONE (2026-08-09)** — `ROOM_TOKEN_SECRET` dev-fallback removed on both sides (server refuses to start without it via `services/auth.py`; frontend mints via `lib/roomToken.ts`); `POST /room` now requires a bearer JWT and derives identity from claims (body identity ignored), with a per-user Redis rate limit (`services/rate_limit.py`, `ROOM_CREATE_RATE_LIMIT`/`ROOM_CREATE_RATE_WINDOW`); room-code collision now raises `RoomCodeAllocationError`→503 instead of overwriting; `/health` pings Redis (503 on failure). Railway deploy is now unblocked. P2 data reliability: weekly scrape commits with no schema validation or failure alerting; kujimap match threshold 0.45 risks silent wrong counts; ラストワン賞 pool inclusion actually diverges between `ticket_pool.py` (+1) and frontend `core.ts` (excluded) — needs a shared golden-vector test.
